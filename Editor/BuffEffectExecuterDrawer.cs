...
namespace TechCosmos.GBF.Editor
{
    ...
    public class BuffEffectExecuterDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            property.isExpanded = EditorGUI.Foldout(foldoutRect, property.isExpanded,
                $"{label.text} — 模式: {modeName}（{effectCount} 个效果）", true);
            ...
            if (property.isExpanded)
            {
                ...
                if (modeProp != null)
                {
                    ...
                    EditorGUI.PropertyField(r, modeProp, new GUIContent("执行模式"), true);
                    y += h + 2;
                }
                ...
                if (effectsProp != null)
                {
                    ...
                    var r = new Rect(position.x, y, position.width, h);
                    EditorGUI.PropertyField(r, effectsProp, new GUIContent("效果列表"), true);
                }
    ...
    public class BuffEffectDrawer : PropertyDrawer
    {
        ...
        private void ShowAddMenu(SerializedProperty property)
        {
            ...
            if (types.Count == 0)
            {
                menu.AddDisabledItem(new GUIContent("没有可用的 BuffEffect"));
            }
            else
            {
                var grouped = types
