...
namespace TechCosmos.GBF.Editor
{
    public class BuffEditorWindow : EditorWindow
    {
        ...
        [MenuItem("Tech-Cosmos/GBF/Buff Editor Window")]
        ...
        [MenuItem("Tech-Cosmos/GBF/Open Buff Editor", true)]
        private static bool OpenWindowValidate() => Selection.activeObject is BuffDataSO;

        [MenuItem("Tech-Cosmos/GBF/Open Buff Editor", priority = 5)]
        ...
        private Type GetBuffModifyEnumType()
        {
            if (!_modifyEnumSearched)
            {
                ...
                {
                    var type = asm.GetType("TechCosmos.GBF.Runtime.BuffModifyType");
        ...
        private Type GetBuffActionEnumType()
        {
            if (!_actionEnumSearched)
            {
                ...
                foreach (var asm in AppDomain.CurrentDomain.GetAssemblies())
                {
                    var type = asm.GetType("TechCosmos.GBF.Runtime.BuffActionType");
